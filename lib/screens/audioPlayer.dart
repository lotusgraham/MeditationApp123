...
class AudioPlayerDemoState extends State<AudioPlayerDemo> {
  ...
  Duration buffering = Duration();
  StreamSubscription<AudioPlaybackState> _playerStateSubscription;
  // StreamSubscription<AudioPlayerState> _playerStateSubscription;
  StreamSubscription<Duration> _playerPositionController;
  StreamSubscription<Duration> _playerBufferingSubscription;
  StreamSubscription<AudioPlaybackEvent> _playerErrorSubscription;
  ...
  void initState() {
    ...
    _playerStateSubscription =
        audioPlayer.playbackStateStream.listen((AudioPlaybackState state) {
      if (mounted) setState(() => this.state = state);
    });

    _playerPositionController =
        audioPlayer.bufferedPositionStream.listen((Duration position) {
      updateTime(position);
      if (mounted) setState(() => this.position = position);
    });

    _playerBufferingSubscription =
        audioPlayer.bufferedPositionStream.listen((Duration percent) {
      ...
      case AudioPlaybackState.connecting:
        {
          ...
          if (buffering.inSeconds > 5 && state != AudioPlaybackState.completed)
            onPlay();
          break;
        }
  ...
  updateTime(Duration timer) {
    if (watch.isRunning) {
      int hundreds = (timer.inMilliseconds / 10).truncate();
